...
function applyMasksToWall(e, t, o) {
  ...
  if (
    (Object.keys(Threed).forEach(function (e) {
      if (Threed[e].useMask && Threed[e].data.level === o) {
        ...
        if (n.intersectsBox(r)) {
          i = !0
          var s = new ThreeBSP(maskObjects[e])
          if (null === l) l = new ThreeBSP(t)
          else {
            var d = new THREE.Mesh(a)
    ...
    var r = new THREE.Mesh(a, wallMaterial)
...
function applyMasksToRoof(e, t, o) {
  ...
  if (
    ...
    var r = new THREE.Mesh(a, roofMaterial)
    r.position.copy(t.position),
...
function loadDimension(e) {
  ...
  var t = dimensionPath.data.adjacent,
                                  ...
                                  (p.onMouseDown = function (t) {
                                    if ("pointer" === toolMode) {
                                      deselectAll(),
                                        ...
                                        this.data.toolsRectangleInner.remove(),
                                        (this.rotation = 0)
                                      var o = new paper.Path.Rectangle(
                                        this.bounds
                                      )
                                        ; (this.rotation = this.data.angle),
                                          (o.data.type = "toolsRectangle"),
...
function redrawGrid() {
  if (!redrawing && "3dView" != UILayout) {
    ...
    xLines.forEach(function (e) {
      ...
      var l = parseInt(e.segments[0].point.x)
      0 === l
    ...
    var l = paper.view.bounds.top % t
      ; (n = 0),
        yLines.forEach(function (e) {
          ...
          var a = parseInt(e.segments[0].point.y)
          0 === a
            ? ((e.style.strokeColor = "white"),
...
function generateShareLink() {
  try {
    createThumbForHistory(),
      $.ajax({
        type: "POST",
        url: "https://threedgarden.com/demo/api/getsharelink",
        ...
        success: function (e) {
          console.debug(e),
            ...
            (document.getElementById("shareLinkUrl3d").value =
              "https://threedgarden.com/demo/3d/" + shareLinkUrl),
            (document.getElementById("shareLinkUrlPlan").value =
...
function initPlanView() {
  ...
  var s = new Rectangle(new Point(0, 0), new Point(0, 0))
...
function drawAngle(e, t, o, a) {
  ...
  var d = o.add(new Point(Math.cos(-t) * n, -Math.sin(-t) * n)),
    c = new paper.Path.Arc(i, s, d).removeOnMove()
  c.style = { strokeColor: "white" }
  var u = new paper.PointText(s.add(new paper.Point(0, 3))).removeOnMove()
...
function reDrawWallCorners(e) {
  if (2 === e.segments.length) {
    var t = e.data.thickness / 2
    if (e.data.join0.id) {
      var o
      if (0 === e.data.join0.seg) {
        var a = Walls[e.data.join0.id].data.thickness / 2
        o = Walls[e.data.join0.id].segments[1].point
        var n = e.segments[0].point,
        ...
        var v = wallsRectangles3d[e.data.id],
        ...
        var d = r + Math.PI / 2,
          ...
          ; (wallsRectangles[e.data.id].segments[0].point = g),
            ...
            (wallsRectangles[e.data.id].closed = !0),
            (wallsRectangles[e.data.join0.id].closed = !0)
        var w = wallsRectangles3d[e.data.id],
    ...
    if (e.data.join1.id) {
      ...
      if (0 === e.data.join1.seg) {
        var a = Walls[e.data.join1.id].data.thickness / 2
        l = Walls[e.data.join1.id].segments[1].point
        var i = getAngleRadians(n, l),
          r = getAngleRadians(o, n),
          s = (i - r + 2 * Math.PI) % (2 * Math.PI)
        s /= 2
        var d = r + Math.PI / 2,
        ...
        var w = wallsRectangles3d[e.data.join1.id],
...
function reDrawRoofCorners(e) {
  if (2 === e.segments.length) {
    var t = e.data.width / 2
    if (e.data.join0.id) {
      var o
      if (0 === e.data.join0.seg) {
        var a = Roofs[e.data.join0.id].data.width / 2
        ...
        var v = roofsRectangles3d[e.data.id],
        ...
        var d = r + Math.PI / 2,
          ...
          ; (roofsRectangles[e.data.id].segments[0].point = g),
            ...
            (roofsRectangles[e.data.id].closed = !0),
            (roofsRectangles[e.data.join0.id].closed = !0)
        var w = roofsRectangles3d[e.data.id],
    ...
    if (e.data.join1.id) {
      var l,
        ...
        n = e.segments[1].point
      if (0 === e.data.join1.seg) {
        var a = Roofs[e.data.join1.id].data.width / 2
        ...
        var w = roofsRectangles3d[e.data.join1.id],
                ...
                ; (backgroundRaster = new paper.Raster(e)),
                  ...
                  (backgroundRaster.onLoad = function () {
                    deselectAll(),
                      ...
                      (selectedItem = backgroundRaster)
                    var e = new paper.Path.Rectangle(backgroundRaster.bounds)
...
function drawDimension(e, t, o) {
  ...
  n.lineBy(a.normalize(a.length / 2)),
    ...
    n.lineBy(a.normalize(o).rotate(90))
  var i = new paper.PointText({}).removeOnMove()
  Math.abs(a.angle) > 90
...
function updateWallThickness(e) {
  var t = parseInt(document.getElementById("wallIdHidden").value)
  console.dir(Walls[t]),
...
function updateRoofWidth(e) {
  var t = parseInt(document.getElementById("roofIdHidden").value)
...
function updateExtraDefaultRoofInfo() {
  var e = document.getElementById("defaultRoofWidthProp").value,
...
function updateExtraRoofInfo() {
  var e = document.getElementById("roofWidthProp").value,
...
function redrawFloor(e) {
  ...
  var o = new THREE.ExtrudeBufferGeometry(t, extrudeSettings),
  ...
  Object.keys(Threed).forEach(function (t) {
    if (Threed[t].useMask && Threed[t].data.level === e.data.level) {
      ...
      if (l.intersectsBox(o)) {
        r = !0
        var s = new ThreeBSP(maskObjects[t])
        if (null === i) i = new ThreeBSP(a)
        else {
          var d = new THREE.Mesh(n)
          i = new ThreeBSP(d)
        }
    ...
    var t = parseFloat(e)
      ; (clickableObjects[selectedItem.data.id].scale.x =
        ...
        (maskObjects[selectedItem.data.id].scale.x =
          clickableObjects[selectedItem.data.id].scale.x)
    var o = selectedItem.position,
      a = selectedItem.rotation
      ; (selectedItem.rotation = 0),
        (selectedItem.bounds.width = Math.abs(t)),
        selectedItem.data.toolsRectangleInner &&
        selectedItem.data.toolsRectangleInner.remove()
    var n = new paper.Path.Rectangle(selectedItem.bounds)
...
function copyShareLink() {
  var e = document.getElementById("shareLinkUrl")
  e.select(), document.execCommand("copy")
}
function copyShareLink3d() {
  var e = document.getElementById("shareLinkUrl3d")
  e.select(), document.execCommand("copy")
}
function copyShareLinkPlan() {
  var e = document.getElementById("shareLinkUrlPlan")
  e.select(), document.execCommand("copy")
}
function exportToObj() {
  try {
    ...
    for (
      a.download = "ThreeDHomeGarden.obj",
...
function recenter3dview() {
  var e = 1,
    ...
    d = 0
  Object.keys(wallsRectangles3d).forEach(function (e) {
    var r = wallsRectangles3d[e]
    if ("object" == typeof r) {
      var s = new THREE.Box3().setFromObject(r)
      s.min.x < t && (t = s.min.x),
        s.max.x > o && (o = s.max.x),
        s.min.z < a && (a = s.min.z),
        s.max.z > n && (n = s.max.z),
...
function recenterPlanView() {
  ...
  var a = paper.view.bounds.width * paper.view.bounds.height,
    n = Math.max(
  ...
  var i = zoomRectangle.bounds.center.x,
    ...
    try {
      if ("editable" !== document.activeElement.className) {
        ...
        if (17 === e.keyCode) setCtrlKeyPressed(!0)
          ...
          var o = paper.view.bounds.width * paper.view.bounds.height,
            a = Math.max(
