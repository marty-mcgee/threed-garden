...
// @ts-expect-error
import box from "/assets/farmbot/3D/models/box.glb"
// @ts-expect-error
import btn from "/assets/farmbot/3D/models/push_button.glb"
// @ts-expect-error
import led from "/assets/farmbot/3D/models/led_indicator.glb"

import * as THREE from "three"

// const DRACO_LIB_DIR = "../farmbot-3d-demo-react/assets/farmbot/3D/lib/"
// const DRACO_LIB_DIR = "../assets/farmbot/3D/lib/"
...
Object.values(MODELS).map(model => useGLTF.preload(model, DRACO_LIB_DIR))
...
export const ElectronicsBox = (props: BoxTopBaseProps) => {
  const box = useGLTF(MODELS.box, DRACO_LIB_DIR) as Box
  const btn = useGLTF(MODELS.btn, DRACO_LIB_DIR) as Btn
  const led = useGLTF(MODELS.led, DRACO_LIB_DIR) as Led
  const SCALE = 1000
