...
const JoystickComponents = (props: EcctrlJoystickProps) => {
   ...
   */
  const joystickBaseGeo = useMemo(() => new THREE.CylinderGeometry(2.0, 1.8, 0.4, 8), [])
   ...
   */
  const joystickBaseMaterial = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.2 }), [])
  const joystickStickMaterial = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.2 }), [])
  const joystickHandleMaterial = useMemo(() => new THREE.MeshNormalMaterial({ transparent: true, opacity: 0.6 }), [])
