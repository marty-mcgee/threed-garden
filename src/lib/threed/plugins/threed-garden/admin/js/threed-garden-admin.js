...
/** REST API URLS */
const API_URL_SCENES = `${restURL}threed_scene/?_embed&per_page=100`
const API_URL_ALLOTMENTS = `${restURL}threed_allotment/?_embed&per_page=100`
const API_URL_BEDS = `${restURL}threed_bed/?_embed&per_page=100`
const API_URL_PLANTING_PLANS = `${restURL}threed_planting_plan/?_embed&per_page=100`
const API_URL_PLANTS = `${restURL}threed_plant/?_embed&per_page=100`
...
function buildScene() {
	...
	if ( wpScene.scene_background_image_px ) {
		let cubeMapURLs = [
			wpScene.scene_background_image_px,
			wpScene.scene_background_image_nx,
			wpScene.scene_background_image_py,
			wpScene.scene_background_image_ny,
			wpScene.scene_background_image_pz,
			wpScene.scene_background_image_nz
		]
	...
	else if ( wpScene.scene_background_image ) {
		// let bgTexture = loaderTexture.load(wpScene.scene_background_image)
		// scene.background = bgTexture
		let bgTexture = loaderTexture.load(
			wpScene.scene_background_image.guid,
			() => {
	...
	else if ( wpScene.scene_background_color ) {
		scene.background = new THREE.Color(wpScene.scene_background_color)
	...
	let plane = getPlane(
		wpScene.scene_plane_width_x, 
		wpScene.scene_plane_length_y, 
		wpScene.scene_plane_background_color
	)
	...
	if ( wpScene.scene_plane_texture_image ) {
		...
		plane.material.map = loaderTexture.load(wpScene.scene_plane_texture_image)
		// plane.material.bumpMap = loaderTexture.load("/wp-content/plugins/threed-garden/admin/media/textures/grasslight-big-nm.jpg")
		// plane.material.bumpMap = loaderTexture.load(wpScene.scene_plane_texture_image)
...
function loadFarmHouseGLTF(plane) {
	...
	loaderGLTF.load( `${params.assetsPath}gltf/Residential House.glb`, function (object) {
		...
		console.debug("-----------------------")
		console.debug("loadFarmHouse object----------------")
		console.debug(object)
		console.debug("-----------------------")

		console.debug("-----------------------")
		console.debug("loadFarmHouse model----------------")
		console.debug(model)
		console.debug("-----------------------")
...
function loadChicken(plane) {
	...
	loaderGLTF.load( `${params.assetsPath}gltf/Chicken.glb`, function (object) {
		...
		console.debug("-----------------------")
		console.debug("loadChicken object----------------")
		console.debug(object)
		console.debug("-----------------------")

		console.debug("-----------------------")
		console.debug("loadChicken model----------------")
		console.debug(model)
		console.debug("-----------------------")
...
function loadChickGLTF(plane) {
	...
	loaderGLTF.load( `${params.assetsPath}gltf/Chick.glb`, function (object) {
		...
		console.debug("-----------------------")
		console.debug("loadChickGLTF object----------------")
		console.debug(object)
		console.debug("-----------------------")

		console.debug("-----------------------")
		console.debug("loadChickGLTF model----------------")
		console.debug(model)
		console.debug("-----------------------")
		...
		loaderTexture.load(`${params.assetsPath}Hen&Chicken_FBX/Textures/Hen&Chicken_A.png`, function(texture) {
			object.traverse( function ( child ) {
				if ( child.isMesh ){
					...
					console.debug("-----------------------")
					console.debug("loadHen child----------------")
					console.debug(child)
					console.debug(child.geometry.attributes.uv)
					console.debug("-----------------------")
				}
			} )
		})
...
function loadHenGLTF(plane) {
	...
	loaderGLTF.load( `${params.assetsPath}gltf/Hen_HP.glb`, function (object) {
		...
		console.debug("-----------------------")
		console.debug("loadHenGLTF object----------------")
		console.debug(object)
		console.debug("-----------------------")

		console.debug("-----------------------")
		console.debug("loadHenGLTF model----------------")
		console.debug(model)
		console.debug("-----------------------")
...
function buildAllotments(postObject, plane, sceneID) {
	...
	// console.debug("-------------------------")
	// console.debug("postObject ALLOTMENTS----")
	// console.debug(postObject)
	// console.debug("-------------------------")
	
	var filteredPostObject = postObject.filter(function (obj) {
		return obj.allotment_scene == sceneID
	})
	// console.debug("filteredPostObject-------")
	// console.debug(filteredPostObject)
	// console.debug("-------------------------")
	...
	filteredPostObject.forEach( function(key) {
		...
		// console.debug("-------------------------")
		// console.debug("key.id (postObject)------")
		...
		allotment.position = {}
		allotment.images = {}
		allotment.parameters.x = parseInt(key.allotment_width)
		allotment.parameters.y = parseInt(key.allotment_length)
		allotment.parameters.z = parseInt(key.allotment_height) // 0
		allotment.position.x = parseInt(key.allotment_position_x)
		allotment.position.y = parseInt(key.allotment_position_y)
		allotment.position.z = parseInt(key.allotment_position_z)
		allotment.images.texture = key.allotment_texture_image
		allotment.images.featured = getFeaturedImage(key)
		allotment.shape = key.allotment_shape
		allotment.color = key.allotment_color
...
function buildBeds(postObject, plane, allotmentID, posOffsetX, posOffsetY, posOffsetZ) {
	...
	// console.debug("-------------------------")
	// console.debug("postObject BEDS----------")
	// console.debug(postObject)
	// console.debug("-------------------------")
	
	// only show beds for this allotment structure
	var filteredPostObject = postObject.filter(function (obj) {
		return obj.bed_allotment == allotmentID
	})
	// console.debug("filteredPostObject-------")
	// console.debug(filteredPostObject)
	// console.debug("-------------------------")
	...
	filteredPostObject.forEach( function(key) {
		...
		bed.position = {}
		bed.images = {}
		bed.parameters.x = parseInt(key.bed_width) / 12
		bed.parameters.y = parseInt(key.bed_length) / 12
		bed.parameters.z = parseInt(key.bed_height) / 12
		bed.position.x = parseInt(key.bed_position_x) / 12 + posOffsetX
		bed.position.y = parseInt(key.bed_position_y) / 12 + posOffsetY
		bed.position.z = parseInt(key.bed_position_z) / 12 + (bed.parameters.z / 2) // + posOffsetZ
		bed.images.texture = key.bed_texture_image
		bed.images.featured = getFeaturedImage(key)
		bed.shape = key.bed_shape
		bed.color = key.bed_color
...
function buildPlantingPlans(postObject, plane, bedID, posOffsetX, posOffsetY, posOffsetZ) {
	...
	// console.debug("-------------------------")
	// console.debug("postObject PLANTING PLANS")
	// console.debug(postObject)
	// console.debug("-------------------------")
	...
	postObject.forEach(function (obj) {
		obj.planting_plan_bed_plant_schedule.forEach(function(i) {
			if ( i.planting_plan_bed == bedID ) {
				//console.debug("MATCHED at: ", i)
				//matches.push(i)
				//matches.push(obj)
	...
	filteredPostObject.forEach( function(key) {
		...
		// console.debug("-------------------------")
		// console.debug("key.id (filteredPostObject)------")
		// console.debug(key.id)
		// console.debug(key)
		// console.debug("-------------------------")

		// for each planting plan bed-plant schedule..
		key.planting_plan_bed_plant_schedule.forEach(function(key2) {
			...
			if ( key2.planting_plan_bed == bedID ) {
				...
				filteredPlant.forEach(function(key3) {
					...
					plant.position = {}
					plant.images = {}
					plant.parameters.x = parseInt(key3.plant_width) / 12
					plant.parameters.y = parseInt(key3.plant_length) / 12
					plant.parameters.z = parseInt(key3.plant_height) / 12
					...
					plant.images.texture = key3.plant_texture_image
					plant.images.featured = getFeaturedImage(key)
					plant.shape = key3.plant_shape
					plant.color = key3.plant_color
...
function watchPointer(camera, targetList){
	...
	if ( intersects.length > 0 ) {
		...
		if ( intersects[0].object != params.intersectedObject1 ) {
			...
			// console.debug("-------------------------")
			// console.debug("params.intersectedObject1-------------")
			// console.debug(params.intersectedObject1)
			// console.debug("-------------------------")
...
function onPointerMove( event ) {
	...
	// console.debug("------------------")
	// console.debug("pointer hover-------")
	// console.debug(pointer.x, pointer.y)
	// console.debug("------------------")
}
...
function onPointerDown(event) {
	...
	if ( intersects.length > 0 )
	{
		//console.debug("Hit @ " + toString( intersects[0].point ) )
		...
		// 	console.debug("-----------------------------")
		// 	console.debug("params.intersectedObject2 null------------")
		// 	console.debug("theIntersectedObject NEW--------")
		// 	console.debug(theIntersectedObject)
		// 	console.debug("-----------------------------")
		// }
		// else {
		// 	console.debug("-----------------------------")
		// 	console.debug("params.intersectedObject2 already stored--")
		// 	console.debug("theIntersectedObject -----------")
		// 	console.debug(theIntersectedObject)
		// 	console.debug("-----------------------------")
		// }
		...
		// }
		// console.debug("------------------")
		// console.debug("event.target.controls--------")
		// console.debug(event.target.controls)
		// console.debug("------------------")
		...
		params.intersectedObject2.children.forEach( function(key) {
			// console.debug("--------------------------------------")
			// console.debug("key (pre-process)------")
			// console.debug(`key.type: ${key.type}`)
			// console.debug(`key.visible: ${key.visible}`)
			// console.debug(`key.element.hidden: ${key.element.hidden}`)
			// console.debug(key)
			// console.debug("--------------------------------------")
			...
			if ( key.type === "Object3D" && event.button == 0 ) {
				...
				//if (key.visible == false) {
					// console.debug("-------------------------")
					// console.debug("TRUE------")
					// console.debug(key.element.hidden)
					// console.debug("-------------------------")
				...
				// console.debug("--------------------------------------")
				// console.debug("key (post-process)------")
				// console.debug(`key.type: ${key.type}`)
				// console.debug(`key.visible: ${key.visible}`)
				// console.debug(`key.element.hidden: ${key.element.hidden}`)
				// console.debug(key)
		...
		if ( params.intersectedObject2.userData.annotation ) {
			console.debug("------------------")
			console.debug("ANNOTATION ------------")
			console.debug(params.intersectedObject2.userData.annotation)
			console.debug("------------------")
		}
		else {
			console.debug("------------------")
			console.debug("ANNOTATION? ------------")
			console.debug(params.intersectedObject2.userData)
			console.debug("------------------")
		}
		*/
...
function panCam(xTarget, yTarget, zTarget, tweenDuration, camera, controls){
	...
	var camTween = new TWEEN.Tween(camera.position)
		...
		.onComplete(function() {
			...
			console.debug("onComplete camera---------")
		})
		.start()
	var targetTween = new TWEEN.Tween(controls.target)
		...
		.onComplete(function() {
			...
			console.debug("onComplete controls---------")
		})
		.start()

	console.debug("------------------")
	console.debug("camTween, targetTween---------")
	console.debug(camTween)
	console.debug(targetTween)
	console.debug("------------------")
