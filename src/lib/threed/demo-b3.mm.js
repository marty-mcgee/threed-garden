...
export function deselectAll() {
  try {
    ;(mouseMode = -1),
      selectedItem &&
        selectedItem.data &&
        (selectedItem.data.toolsRectangleInner && (selectedItem.data.toolsRectangleInner.visible = !1),
        selectedItem.data.boxHelper && (selectedItem.data.boxHelper.visible = !1),
...
export function resetPlan() {
  ...
  ;(project.activeLayer.name = 'level0'), (project.activeLayer.data = { id: '0', height: 0 })
  try {
    ;(floorsGroup = {}),
...
export function doUndo() {
  if (planHistoryPosition > 0) {
    ...
    plan.furnitureAddedKey
      ...
      ? (loadFurniture(e.furniture[plan.furnitureDeletedKey], !1, !1), updatePlan(e))
      : plan.wallAddedKey || 0 === plan.wallAddedKey
      ? (deleteWallByKey(plan.wallAddedKey), updatePlan(e))
      : plan.wallDirtyKey || 0 === plan.wallDirtyKey
      ? (deleteWallByKey(plan.wallDirtyKey), loadWall(e.walls[plan.wallDirtyKey]), updatePlan(e))
      ...
      ? (deleteRoofByKey(plan.roofDirtyKey), loadRoof(e.roofs[plan.roofDirtyKey]), updatePlan(e))
      ...
      ? (setEndDrawingFloors(),
        ...
        Floors[plan.floorDirtyKey].segments.length > 1 &&
          ...
          (floorHelperPath.segments[0].point =
            Floors[plan.floorDirtyKey].segments[Floors[plan.floorDirtyKey].segments.length - 1].point),
      ...
      ? (loadFloor(e.floors[plan.floorDeletedKey]),
        2 === Floors[plan.floorDeletedKey].segments.length &&
          ...
          (floorHelperPath.visible = !0),
          (floorHelperPath.segments[0].point =
            Floors[plan.floorDeletedKey].segments[Floors[plan.floorDeletedKey].segments.length - 1].point),
          (floorHelperPath.segments[1].point = lastMousePoint),
      ...
      : plan.dimensionEditedKey ||
        (plan.dimensionDeletedKey
          ? (loadDimension(e.dimensions[plan.dimensionDeletedKey]), updatePlan(e))
          : plan.textAddedKey
          ? (deleteTextByKey(plan.textAddedKey), updatePlan(e))
          : plan.textEditedKey
          ? (deleteTextByKey(plan.textEditedKey), loadText(e.texts[plan.textEditedKey]), updatePlan(e))
          : plan.textDeletedKey
...
export function doRedo() {
  if (planHistory.length > planHistoryPosition + 1) {
    planHistoryPosition++, deselectAll()
    let e = JSON.parse(planHistory[planHistoryPosition])
    if (e.furnitureAddedKey) loadFurniture(e.furniture[e.furnitureAddedKey], !1, !1), updatePlan(e)
    else if (e.furnitureDirtyKey) editFurnitureByKey(e.furniture[e.furnitureDirtyKey]), updatePlan(e)
    else if (e.furnitureDeletedKey) deleteFurnitureByKey(e.furnitureDeletedKey), updatePlan(e)
    else if (e.wallAddedKey || 0 === e.wallAddedKey) loadWall(e.walls[e.wallAddedKey]), updatePlan(e)
    else if (e.wallDirtyKey || 0 === e.wallDirtyKey)
      deleteWallByKey(e.wallDirtyKey), loadWall(e.walls[e.wallDirtyKey]), updatePlan(e)
    else if (e.wallDeletedKey || 0 === e.wallDeletedKey) deleteWallByKey(e.wallDeletedKey), updatePlan(e)
    else if (e.roofAddedKey || 0 === e.roofAddedKey) loadRoof(e.roofs[e.roofAddedKey]), updatePlan(e)
    else if (e.roofDirtyKey || 0 === e.roofDirtyKey)
      deleteRoofByKey(e.roofDirtyKey), loadRoof(e.roofs[e.roofDirtyKey]), updatePlan(e)
    else if (e.roofDeletedKey || 0 === e.roofDeletedKey) deleteRoofByKey(e.roofDeletedKey), updatePlan(e)
      ...
      'floor' !== toolMode && setToolMode('floor'),
        ...
        2 === Floors[e.floorAddedKey].segments.length &&
          ((floorHelperPath.visible = !0),
          (floorHelperPath.segments[0].point =
            Floors[e.floorAddedKey].segments[Floors[e.floorAddedKey].segments.length - 1].point),
          (floorHelperPath.segments[1].point = lastMousePoint),
        ...
        ? ((t = JSON.parse(planHistory[planHistoryPosition + 1])),
          null === t.floorDirtyKey
            ...
            : ((startedDrawingFloor = !0),
              ...
              (floorHelperPath.visible = !0),
              (floorHelperPath.segments[0].point =
                Floors[e.floorDirtyKey].segments[Floors[e.floorDirtyKey].segments.length - 1].point),
              (floorHelperPath.segments[1].point = lastMousePoint),
      ...
      e.floorDeletedKey
        ? ('floor' !== toolMode && setToolMode('floor'), deleteFloorByKey(e.floorDeletedKey), updatePlan(e))
        : e.dimensionAddedKey
        ...
        : e.dimensionEditedKey ||
          (e.dimensionDeletedKey
            ...
            : e.textEditedKey
            ? (deleteTextByKey(e.textEditedKey), loadText(e.texts[e.textEditedKey]), updatePlan(e))
            : e.textDeletedKey
...
function addVerticalGuide() {
  const e = paper.view.viewToProject(
    new paper.Point(event.pageX - planView.offsetLeft, event.pageY - planView.offsetTop)
  )
...
function initThreeJS() {
  ...
  let e = new THREE.PlaneGeometry(100 * groundWidth, 100 * groundLength)
  ;(groundMat = new THREE.MeshPhongMaterial({ transparent: !0, opacity: 1 })),
    ...
    renderer.setSize(parseInt(container.clientWidth), parseInt(container.clientHeight)),
    (renderer.autoClear = !1),
    (renderer.sortObjects = !1),
    (renderer.domElement.ondblclick = onDoubleClick),
    (camera = new THREE.PerspectiveCamera(45, container.innerWidth / container.innerHeight, 10, 25e3)),
    (camera.position.x = 0),
  ...
  ;(sunSphere.position.x = n * Math.cos(a)),
    ...
    scene.add(dirLight)
  let l = new THREE.BoxGeometry(defaultWallThickness, defaultWallHeight, defaultWallThickness),
  ...
  ;(wallHelper3dCube = new THREE.Mesh(l, i)),
    ...
    (l = new THREE.BoxGeometry(defaultRoofWidth, defaultRoofThickness, defaultRoofWidth)),
    (i = new THREE.MeshStandardMaterial({
      color: 0,
      transparent: !0,
      opacity: 0.25,
  ...
  s.color.setHSL(0.6, 1, 0.6), s.groundColor.setHSL(0.095, 1, 0.75), s.position.set(0, 50, 0), model3dScene.add(s)
...
function showModel3dView() {
  ...
  e !== -1 &&
    ...
    new THREE.MTLLoader().setPath('objects/').load(e + '/' + e + '.mtl', function (t) {
      ;(t.baseUrl = 'objects/' + e + '/'),
        t.preload(),
        new THREE.OBJLoader()
          .setMaterials(t)
              ...
              try {
                ...
                for (let a = 0; a < t.children.length; a++) {
                  let n = o.min.x + (o.max.x - o.min.x) / 2,
                    l = o.min.y + (o.max.y - o.min.y) / 2 - (o.max.y - o.min.y) / 2,
                ...
                ;(s.onreadystatechange = function () {
                  if (4 === this.readyState && 200 === this.status) {
                    ...
                    t.forEach(function (e) {
                      ...
                      (document.getElementById('modalModel3dObjHeader').value = r)
                  }
                }),
      ...
      if (!o) {
        ...
        if (e.distance > 1) {
          if (parseInt(e.object.parent.name)) {
            ...
            Floors[a].data.level === project.activeLayer.data.id && ((o = !0), (t = 'floor'))
          } else if (e.object.name.startsWith('roof')) {
            let n = parseInt(e.object.name.substring(4))
            Roofs[n].data.level === project.activeLayer.data.id && ((o = !0), (t = 'roof'))
...
function showAbout() {
  $('.supportEmail').text('support@companyjuice.com'), $('#aboutModal').show(), hideMouseIndicators()
}
...
function enableResizeBackgroundTemplate() {
  deselectAll(),
    backgroundRaster &&
      ...
      (resizeIcon.position = backgroundRaster.data.toolsRectangleInner.segments[3].point),
...
function openTab(e) {
  ...
  for (a = document.getElementsByClassName('tablinks'), t = 0; t < a.length; t++)
    a[t].className = a[t].className.replace(' active', '')
  ;(document.getElementById(e + 'Content').style.display = 'block'), document.getElementById(e).classList.add('active')
}
function doCopy() {
...
function deleteSelectedItem() {
  try {
    if (!readOnly && selectedItem && selectedItem.data.id)
      ...
      } else console.log('delete not implemented for : ' + selectedItem.data.type)
  } catch (e) {
  ...
  if (!readOnly)
    try {
      Object.keys(Texts).forEach(function (t) {
        let o = Texts[t]
        'object' === typeof o && o.data.id.toString() === e.toString() && (Texts[t].remove(), delete Texts[t])
      }),
        delete plan.texts[e]
    } catch (e) {
    ...
    try {
      ;(wallPath = null),
        Object.keys(Walls).forEach(function (t) {
          let o = Walls[t]
          'object' === typeof o && o.data.id.toString() === e.toString() && (Walls[t].remove(), delete Walls[t])
        }),
    ...
    try {
      ;(roofPath = null),
        Object.keys(Roofs).forEach(function (t) {
          let o = Roofs[t]
          'object' === typeof o && o.data.id.toString() === e.toString() && (Roofs[t].remove(), delete Roofs[t])
        }),
        ...
        Object.keys(maskObjectsAppliedRoof).forEach(function (t) {
          'object' === typeof maskObjectsAppliedRoof[t] &&
            ...
            (scene.remove(maskObjectsAppliedRoof[t]), delete maskObjectsAppliedRoof[t])
        }),
        delete plan.roofs[e]
    } catch (e) {
    ...
    try {
      scene.remove(maskObjects[e]),
        Object.keys(maskObjects).forEach(function (t) {
          let o = maskObjects[t]
          'object' === typeof o && o.name.toString() === 'mask' + e.toString() && delete maskObjects[t]
        }),
        scene.remove(clickableObjects[e]),
        Object.keys(clickableObjects).forEach(function (t) {
          let o = clickableObjects[t]
          'object' === typeof o && o.name.toString() === e.toString() && delete clickableObjects[t]
        }),
        Object.keys(Furniture).forEach(function (t) {
          let o = Furniture[t]
          'object' === typeof o &&
            o.data.id.toString() === e.toString() &&
            (Furniture[t].data.toolsRectangleInner && Furniture[t].data.toolsRectangleInner.remove(),
            Furniture[t].remove(),
            delete Furniture[t])
        }),
...
function updatePlanHistory(e, t, o, a, n, l, i, r, s, d, c, u, p, m, g, y, f, h, v) {
...
function savePlan() {
  try {
    ...
    ;(e.download = 'HomeIdea3D.dat'),
      ...
      (e.href = window.URL.createObjectURL(new Blob([JSON.stringify(plan)], { type: 'text/plain' }))),
  ...
  setInterfacePropertiesFromPlan(e),
    e.levels
      ? ((plan.levels = e.levels),
        Object.keys(e.levels).forEach(function (t) {
          'object' === typeof e.levels[t] && ((levelButtons && levelButtons[t]) || addNewLevel(t))
        }))
    ...
    e.furniture &&
      Object.keys(e.furniture).forEach(function (t) {
        ...
        'object' === typeof o &&
          ...
          (loadingProgressTxt =
            'Loading Furniture id : ' + o.id + ' (' + camelCaseToSentence(o.fid) + ')\n' + loadingProgressTxt),
          (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
          loadFurniture(o, !1, !1))
      }),
    ...
    e.walls &&
      Object.keys(e.walls).forEach(function (t) {
        let o = e.walls[t]
        'object' === typeof o &&
          ((loadingProgressTxt = 'Loading Wall Id : ' + o.id + '\n' + loadingProgressTxt),
          (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
          loadWall(o))
      }),
    e.roofs &&
      Object.keys(e.roofs).forEach(function (t) {
        let o = e.roofs[t]
        'object' === typeof o &&
          ((loadingProgressTxt = 'Loading Roof Id : ' + o.id + '\n' + loadingProgressTxt),
          (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
          loadRoof(o))
      }),
    e.floors &&
      Object.keys(e.floors).forEach(function (t) {
        let o = e.floors[t]
        'object' === typeof o &&
          ((loadingProgressTxt = 'Loading Floor Id : ' + o.id + '\n' + loadingProgressTxt),
          (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
          loadFloor(o))
      }),
    e.dimensions &&
      Object.keys(e.dimensions).forEach(function (t) {
        ...
        'object' === typeof o &&
          ((loadingProgressTxt = 'Loading Dimension Id : ' + o.id + '\n' + loadingProgressTxt),
          (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
          loadDimension(o))
      }),
    e.texts &&
      Object.keys(e.texts).forEach(function (t) {
        let o = e.texts[t]
        'object' === typeof o &&
          ((loadingProgressTxt = 'Loading Text Id : ' + o.id + '\n' + loadingProgressTxt),
          (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
          loadText(o))
      }),
    e.verticalGuides &&
...
function doLoadFinished() {
  ;(plan.furnitureAddedKey = null),
    ...
    planHistory.push(JSON.stringify(plan)),
    project.layers.forEach(function (e) {
      ;(loadingProgressTxt = 'Linking Walls : level ' + e.data.id + '\n' + loadingProgressTxt),
        (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
        relinkWallReferences(e.data.id),
        (loadingProgressTxt = 'Linking Roofs : level ' + e.data.id + '\n' + loadingProgressTxt),
        (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
      ...
      if ('object' === typeof o && o.data.level === e) {
        ...
        o.segments.forEach(function (e) {
          ...
          Object.keys(Walls).forEach(function (n) {
            let l = Walls[n]
            if ('object' === typeof o && o.data.id !== l.data.id && o.data.level === l.data.level) {
              let i = 0
              2 === l.segments.length &&
...
function applyMasksToWall(e, t, o) {
  ...
  if (
    ...
    r.position.copy(t.position),
      ...
      (t.visible = !1)
  } else (t.visible = !0), maskObjectsApplied[e] && (scene.remove(maskObjectsApplied[e]), delete maskObjectsApplied[e])
}
...
function applyMasksToRoof(e, t, o) {
  ...
  if (
    ...
    (t.visible = !0),
      maskObjectsAppliedRoof[e] && (scene.remove(maskObjectsAppliedRoof[e]), delete maskObjectsAppliedRoof[e])
}
function loadText(e) {
...
function loadDimension(e) {
  e.id > dimensionIdCounter && (dimensionIdCounter = e.id),
    ...
    (dimensionPath.data.level = (e.dimensionPath[1].data.level ? e.dimensionPath[1].data.level : 0).toString()),
    project.layers['level' + dimensionPath.data.level] || addNewLevel(dimensionPath.data.level),
    project.activeLayer.data.id !== dimensionPath.data.level && setLevel(dimensionPath.data.level),
  ...
  let t = dimensionPath.data.adjacent,
    o = dimensionPath.segments[1].point.subtract(dimensionPath.segments[0].point),
    a = new paper.Path()
  ;(a.data.id = e.id),
...
function loadFloor(e) {
  e.floorPath[1]
    ? (e.id > floorIdCounter && (floorIdCounter = e.id),
      ...
      (floorPath.data.thickness = e.thickness ? e.thickness : defaultFloorThickness),
      (floorPath.data.level = (e.floorPath[1].data.level ? e.floorPath[1].data.level : 0).toString()),
      project.layers['level' + floorPath.data.level] || addNewLevel(floorPath.data.level),
      project.activeLayer.data.id !== floorPath.data.level && setLevel(floorPath.data.level),
...
function loadWall(e) {
  if (e.wallPath[1]) {
    e.id > wallIdCounter && (wallIdCounter = e.id),
      ...
      (wallPath.data.thickness = e.wallPath[1].data.thickness ? e.wallPath[1].data.thickness : defaultWallThickness),
      Array.isArray(e.wallPath[1].data.height)
        ? (wallPath.data.height = [parseInt(e.wallPath[1].data.height[0]), parseInt(e.wallPath[1].data.height[1])])
        : (wallPath.data.height = e.wallPath[1].data.height
            ? [parseInt(e.wallPath[1].data.height), parseInt(e.wallPath[1].data.height)]
            : [defaultWallHeight, defaultWallHeight]),
      (wallPath.data.level = (e.wallPath[1].data.level ? e.wallPath[1].data.level : 0).toString())
    let t = !0
    if (e.wallPath[1] && e.wallPath[1].segments && 2 === e.wallPath[1].segments.length) {
      ...
      e.wallPath[1].segments.forEach(function (e) {
        if ((wallPath.add(new paper.Point(e[0], e[1])), !t)) {
          ...
          ;(c.data.type = 'wallRectangle'),
            ...
            project.layers['level' + wallPath.data.level] || addNewLevel(wallPath.data.level),
            project.activeLayer.data.id !== wallPath.data.level && setLevel(wallPath.data.level),
            wallsGroup[wallPath.data.level].addChild(wallsRectangles[wallPath.data.id]),
            (wallPath.segments[wallPath.segments.length - 1].data = {
              angleRadians: l,
              id: wallPath.data.id,
            })
          let u = new THREE.BoxGeometry(wallPath.data.thickness, wallPath.data.height[0], wallPath.data.thickness),
            p = new THREE.Mesh(u, wallMaterial)
          ;(p.position.x = 0),
            (p.position.y = wallPath.data.height[0] / 2 + project.layers['level' + wallPath.data.level].data.height),
            (p.position.z = 0),
...
function loadRoof(e) {
  if (e.roofPath[1]) {
    e.id > roofIdCounter && (roofIdCounter = e.id),
      ...
      (roofPath.data.width = parseInt(e.roofPath[1].data.width ? e.roofPath[1].data.width : defaultRoofWidth)),
      (roofPath.data.rise = parseInt(
        e.roofPath[1].data.rise || 0 === e.roofPath[1].data.rise ? e.roofPath[1].data.rise : defaultRoofRise
      )),
      (roofPath.data.startHeight = parseInt(
        e.roofPath[1].data.startHeight ? e.roofPath[1].data.startHeight : defaultRoofStartHeight
      )),
      (roofPath.data.thickness = parseInt(
        e.roofPath[1].data.thickness ? e.roofPath[1].data.thickness : defaultRoofThickness
      )),
      (roofPath.data.level = (e.roofPath[1].data.level ? e.roofPath[1].data.level : 0).toString())
    let t = !0
    if (e.roofPath[1] && e.roofPath[1].segments && 2 === e.roofPath[1].segments.length) {
      ...
      e.roofPath[1].segments.forEach(function (e) {
        if ((roofPath.add(new paper.Point(e[0], e[1])), !t)) {
          ...
          ;(c.data.type = 'roofRectangle'),
            ...
            project.layers['level' + roofPath.data.level] || addNewLevel(roofPath.data.level),
            project.activeLayer.data.id !== roofPath.data.level && setLevel(roofPath.data.level),
            roofsGroup[roofPath.data.level].addChild(roofsRectangles[roofPath.data.id]),
            (roofPath.segments[roofPath.segments.length - 1].data = {
              angleRadians: l,
              id: roofPath.data.id,
            })
          let u = new THREE.BoxGeometry(roofPath.data.width, roofPath.data.thickness, roofPath.data.width),
          ...
          ;(p.position.x = 0),
            ...
            (clickableObjects['roof' + roofPath.data.id] = roofsRectangles3d[roofPath.data.id])
        }
        t = !1
      }),
...
function beginDrag(e, t) {
  try {
    showFurnitureLicenseSummary(t), setToolMode('pointer'), (draggingFurnitureId = t), (draggingFurnitureIcon = !0)
    let o = paper.view.viewToProject(new paper.Point(e.pageX - planView.offsetLeft, e.pageY - planView.offsetTop))
    ;(draggingFurnitureRectangle.position = o),
      furnitureItems[t].scale && furnitureItems[t].scale.x
        ? (draggingFurnitureRectangle.bounds.width = furnitureItems[t].size.x * furnitureItems[t].scale.x)
        : (draggingFurnitureRectangle.bounds.width = furnitureItems[t].size.x),
      furnitureItems[t].scale && furnitureItems[t].scale.z
        ? (draggingFurnitureRectangle.bounds.height = furnitureItems[t].size.z * furnitureItems[t].scale.z)
...
function initFurniture(e) {
  ...
  new THREE.MTLLoader().setPath('objects/').load(t + '/' + t + '.mtl', function (a) {
    ;(a.baseUrl = 'objects/' + t + '/'),
      a.preload(),
      new THREE.OBJLoader()
        .setMaterials(a)
                ...
                (n.onload = function () {
                  ...
                  for (let i = 0; i < a.children.length; i++) {
                    let r = l.min.x + (l.max.x - l.min.x) / 2,
                      s = l.min.y + (l.max.y - l.min.y) / 2 - (l.max.y - l.min.y) / 2,
                      d = l.min.z + (l.max.z - l.min.z) / 2
                    a.children[i].translateX(-r), a.children[i].translateY(-s), a.children[i].translateZ(-d)
                  }
                  let c = new THREE.BoxHelper(a, 16711680)
                  ;(c.material.linewidth = 5),
                    (c.visible = !1),
                    a.add(c),
                    (a.position.y = 0.1 + project.activeLayer.data.height + defaultFloorThickness),
                    (a.position.x = e.x),
                    (a.position.z = e.y),
                    scene.add(a),
                    clickableObjectsCounter++
                  ...
                  let p = new THREE.BoxGeometry(a.userData.width, a.userData.height, a.userData.depth),
                  ...
                  ;(m.visible = !1),
                    (m.onLoad = function () {
                      if (
                        ...
                        m.useMask = !0
                        let n = new THREE.Mesh(p, new THREE.MeshStandardMaterial({}))
                      ...
                      ;(tween = new TWEEN.Tween(controls.target).to(a.position, 500).onUpdate(render).start()),
                        (m.visible = !0),
                        (Furniture[u] = m),
                        furnitureGroup[project.activeLayer.data.id].addChild(Furniture[u]),
                        (plan.furniture[u] = {
                          id: u,
                          fid: t,
                      ...
                      for (
                        ...
                        (d[g] = 255 - d[g]), (d[g + 1] = 255 - d[g + 1]), (d[g + 2] = 255 - d[g + 2])
...
function loadFurniture(e, t, o) {
  if (furnitureItems[e.fid])
    if ((e.id > clickableObjectsCounter && (clickableObjectsCounter = e.id), 'planView' !== UILayout))
      new THREE.MTLLoader().setPath('objects/').load(e.fid + '/' + e.fid + '.mtl', function (a) {
                  ...
                  ;(n.src = 'objects/' + e.fid + '_top.png'),
                    (n.onload = function () {
                      ...
                      ;(c.material.linewidth = 5),
                        ...
                        (a.name = e.id)
                      let u = new THREE.BoxGeometry(a.userData.width, a.userData.height, a.userData.depth),
                      ...
                      ;(p.visible = !1),
                        ...
                        project.layers['level' + p.data.level] || addNewLevel(p.data.level),
                        project.activeLayer.data.id !== p.data.level && (o || setLevel(p.data.level)),
                        ...
                        (p.onLoad = (function (e) {
                              ...
                              (p.onMouseDown = function (t) {
                                if ('pointer' === toolMode) {
                                  deselectAll(),
                                    (selectedItem = this),
                                    (mouseMode = 0),
                                    (offsetMousePoint = selectedItem.position.subtract(t.point)),
                                    (offsetMousePoint.x = parseInt(offsetMousePoint.x)),
                                    (offsetMousePoint.y = parseInt(offsetMousePoint.y)),
                                    selectedItem.bringToFront(),
                                    this.data.toolsRectangleInner && this.data.toolsRectangleInner.remove(),
                          ...
                          if (furnitureItems[e.fid].useMask) {
                            p.useMask = !0
                            let i = new THREE.Mesh(u, new THREE.MeshStandardMaterial({}))
                            ;(i.position.x = a.position.x),
                              (i.position.y = a.position.y),
                              (i.position.z = a.position.z),
                              i.geometry.translate(0, a.userData.height / 2, 0),
                              (i.visible = !1),
                              scene.add(i),
                              (maskObjects[e.id] = i),
                              (i.name = 'mask' + e.id)
                          ...
                          if (
                            ...
                            (p.position = new paper.Point(parseInt(e.position.x), parseInt(e.position.z))),
                            e.angle)
                          ) {
                            let r = (e.angle + 360) % 360
                            p.rotate(r),
                              (p.data.angle = r),
                              clickableObjects[e.id].rotateY((-r / 180) * Math.PI),
                              maskObjects[e.id] && maskObjects[e.id].rotateY((-r / 180) * Math.PI)
                          } else p.data.angle = 0
                          e.scale &&
                            maskObjects[e.id] &&
                            ...
                            (loadingProgressTxt = 'Furniture id : ' + e.id + ' loaded\n' + loadingProgressTxt),
                            (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
                            loadedFurnitureCount === furnitureToLoadCount && doLoadFinished(),
                            (progressBar.style.display = 'none')
                          for (
                            ...
                            (c[m] = 255 - c[m]), (c[m + 1] = 255 - c[m + 1]), (c[m + 2] = 255 - c[m + 2])
          ...
          ;(t.visible = !1),
            ...
            (t.onLoad = (function (e) {
              if (
                ...
                (t.position = new paper.Point(parseInt(e.position.x), parseInt(e.position.z))),
                e.angle)
              ) {
              ...
              ;(Furniture[e.id] = t),
                ...
                loadedFurnitureCount === furnitureToLoadCount && doLoadFinished()
              for (
                let a = t.canvas.getContext('2d'), n = a.getImageData(0, 0, t.width, t.height), l = n.data, i = 0;
                i < l.length;
                i += 4
              )
                (l[i] = 255 - l[i]), (l[i + 1] = 255 - l[i + 1]), (l[i + 2] = 255 - l[i + 2])
    ...
    loadedFurnitureCount++,
      (loadingProgressTxt =
        '*** Could not find Furniture [' + e.fid + '] for id : ' + e.id + ' ***\n' + loadingProgressTxt),
      (document.getElementById('modalLoadingDataInfo').innerHTML = loadingProgressTxt),
...
function editFurnitureByKey(e) {
  ;(clickableObjects[e.id].position.x = parseFloat(e.position.x)),
    ...
    clickableObjects[e.id].scale.set(parseFloat(e.scale.x), parseFloat(e.scale.y), parseFloat(e.scale.z)),
    ...
    Object.keys(Furniture).forEach(function (t) {
      ...
      if ('object' === typeof o && o.data.id === e.id) {
        ...
        ;(o.rotation = 0),
          ...
          o.rotate(a),
          (o.data.angle = a),
          (o.position = new paper.Point(parseFloat(e.position.x), parseFloat(e.position.z)))
      }
    }),
    render()
}
function redrawGrid() {
  if (!redrawing && '3dView' !== UILayout) {
    if (((redrawing = !0), (screenScale = screenAvg / paper.view.zoom / 75), selectedItem && selectedItem.data))
    ...
    paper.view.zoom < 0.1875
      ? ((t = 200), (o = 2e3), (snapTolerance = 100), (paper.settings.hitTolerance = 3))
      : paper.view.zoom < 0.375
      ? ((t = 100), (o = 1e3), (snapTolerance = 50), (paper.settings.hitTolerance = 3))
      : paper.view.zoom < 0.75
      ? ((t = 50), (o = 500), (snapTolerance = 25), (paper.settings.hitTolerance = 3))
      : paper.view.zoom < 1.5
      ? ((t = 20), (o = 200), (snapTolerance = 10), (paper.settings.hitTolerance = 3))
      : paper.view.zoom < 3
      ? ((t = 10), (o = 100), (snapTolerance = 5), (paper.settings.hitTolerance = 3))
      : paper.view.zoom < 6
      ? ((t = 5), (o = 50), (snapTolerance = 2), (paper.settings.hitTolerance = 3))
      : paper.view.zoom < 12
      ? ((t = 2), (o = 20), (snapTolerance = 1), (paper.settings.hitTolerance = 2))
      : paper.view.zoom < 24 && ((t = 1), (o = 10), (snapTolerance = 0.5), (paper.settings.hitTolerance = 1)),
    ...
    xLines.forEach(function (e) {
      ...
      0 === l
        ? ((e.style.strokeColor = 'white'),
          rulerBottomCtx.fillText('0cm', (l - paper.view.bounds.left) * paper.view.zoom, 14))
        : l % o === 0
    ...
    ;(n = 0),
      yLines.forEach(function (e) {
        ...
        0 === a
          ? ((e.style.strokeColor = 'white'),
            rulerLeftCtx.fillText('0cm', 26, (a - paper.view.bounds.top) * paper.view.zoom + 4))
          : a % o === 0
          ? ((e.style.strokeColor = '#81673a'),
      ...
      $.ajax({
        ...
        success: function (e) {
          console.log(e),
            ...
            (document.getElementById('shareLinkUrl').value = 'https://homeidea3d.seanwasere.com/edit/' + shareLinkUrl),
            (document.getElementById('shareLinkUrl3d').value = 'https://homeidea3d.seanwasere.com/3d/' + shareLinkUrl),
            (document.getElementById('shareLinkUrlPlan').value =
...
function initPlanView() {
  function e(e) {
    ...
    if (t) {
      ...
      if (a > 0.0625 && a < 16) {
        paper.view.zoom = a
        let n = paper.view.viewToProject(new paper.Point(e.pageX - planView.offsetLeft, e.pageY - planView.offsetTop)),
  ...
  if (
    ...
    ;(rulerLeft.oncontextmenu = function () {
      ...
      planCanvas.addEventListener(
        'mousemove',
        function (e) {
          ;(mouseIndicatorX.style.left = e.clientX + 'px'), (mouseIndicatorY.style.top = e.clientY + 'px')
        },
        !1
      )
    let t = /Firefox/i.test(navigator.userAgent) ? 'DOMMouseScroll' : 'mousewheel'
    planCanvas.addEventListener(t, e)
    for (let o = 0; o <= 200; o++) {
      let a = new paper.Point(10 * o, 0),
        n = new paper.Point(10 * o, 100),
        l = new paper.Path.Line(a, n)
      ;(l.strokeColor = '#cccccc'), (l.strokeWidth = 0.5), (l.strokeScaling = !1), xLines.push(l), gridGroup.addChild(l)
    }
    for (let o = 0; o <= 200; o++) {
      let i = new paper.Point(0, 10 * o),
        r = new paper.Point(100, 10 * o),
        l = new paper.Path.Line(i, r)
      ;(l.strokeColor = '#cccccc'), (l.strokeWidth = 0.5), (l.strokeScaling = !1), yLines.push(l), gridGroup.addChild(l)
    }
  ...
  ;(wallHelperPath = new paper.Path.Line(new paper.Point(0, 0), new paper.Point(0, 0))),
    ...
    (roofHelperPath = new paper.Path.Line(new paper.Point(0, 0), new paper.Point(0, 0))),
    ...
    (floorHelperPath = new paper.Path.Line(new paper.Point(0, 0), new paper.Point(0, 0))),
    ...
    (dimensionHelperPath = new paper.Path.Line(new paper.Point(0, 0), new paper.Point(0, 0))),
  ...
  ;(wallHelperRectangle = new Path.Rectangle(s)),
    ...
    (draggingFurnitureRectangle = new paper.Path.Rectangle(new Point(-1, -1), new Point(1, 1))),
    ...
    (draggingFurnitureRectangle.visible = !1),
    furnitureGroup[project.activeLayer.data.id].addChild(draggingFurnitureRectangle),
      ...
      if ('pointer' === toolMode)
              ...
              if (t.item.data.level === project.activeLayer.data.id.toString())
                if ('toolsRectangle' === t.item.data.type)
                  (mouseMode = 0),
                    ...
                    (offsetMousePoint.x = parseInt(offsetMousePoint.x)),
                    (offsetMousePoint.y = parseInt(offsetMousePoint.y))
                else if ('rotateFurnitureTool' === t.item.data.type) (mouseMode = 1), console.log('isRotateTool')
                else if ('stretchFurnitureXZTool' === t.item.data.type) (mouseMode = 2), console.log('isStretchXY')
                  ...
                  (mouseMode = 5),
                    ...
                    elevating
                      ...
                      : ((elevating = !0),
                        (elevateStartHeight = clickableObjects[selectedItem.data.id].position.y),
                  ...
                  for (
                    let l = { x: 0, y: 0, z: 0 },
                      i = Floors3d[selectedItem.data.id].geometry.attributes.position.array,
                      r = 0;
                    r < i.length;
                    r += 3
                  )
                  ...
                  ;(l.x /= i.length / 3),
                    (l.y /= i.length / 3),
                    (l.z /= i.length / 3),
                    (tween = new TWEEN.Tween(controls.target).to(l, 500).onUpdate(render).start()),
                    recalcAllWallCorners()
                } else
                ...
                t.item.data.level === -1 &&
                  ('verticalGuide' === t.item.data.type
                    ...
                    : 'horizontalGuide' === t.item.data.type && ((selectedGuideId = t.item.data.id), (mouseMode = 10)))
        ...
        if (2 === e.event.buttons) mouseMode = -1
          ...
          if (((mouseMode = 0), Date.now() - lastNewWallSegmentClick > 250)) {
            if (
              ...
              (snapPoint.y = parseInt(e.point.y)),
              ctrlKeyPressed &&
                ((snapPoint.x = snapPoint.x - (snapPoint.x % 10)), (snapPoint.y = snapPoint.y - (snapPoint.y % 10))),
              recalcAllUnjoinedWallSegments(wallIdCounter),
              recalcAllWallSegmentsOnOtherLevels(wallIdCounter, project.activeLayer.data.id),
              startedDrawingWalls)
            ) {
              ...
              ctrlKeyPressed && (s.angle = 15 * Math.round(s.angle / 15)),
                ...
                snapPointOverride.id &&
                  ((snapPoint = new paper.Point(snapPointOverride.x, snapPointOverride.y)), (snapPointOverride = {}))
              try {
                ...
                ;(m.data.type = 'wallRectangle'),
                  ...
                  wallsGroup[project.activeLayer.data.id].addChild(wallsRectangles[wallPath.data.id])
                ...
                ;(y.position.x = wallHelper3dCube.position.x),
                  ...
                  wallPath.segments.length > 2 && alert('problem to many segments'),
                  (wallPath = new paper.Path()),
                  (wallPath.strokeColor = new paper.Color(0, 0, 0, 0)),
            ...
            ;(wallHelperPath.segments[0].point = snapPoint),
              ...
              (wallHelper3dCube.geometry.vertices[1].x = wallHelperRectangle.segments[0].point.x),
              (wallHelper3dCube.geometry.vertices[1].z = wallHelperRectangle.segments[0].point.y),
              (wallHelper3dCube.geometry.vertices[3].x = wallHelperRectangle.segments[0].point.x),
              (wallHelper3dCube.geometry.vertices[3].z = wallHelperRectangle.segments[0].point.y),
              (wallHelper3dCube.geometry.vertices[0].x = wallHelperRectangle.segments[1].point.x),
              (wallHelper3dCube.geometry.vertices[0].z = wallHelperRectangle.segments[1].point.y),
              (wallHelper3dCube.geometry.vertices[2].x = wallHelperRectangle.segments[1].point.x),
              (wallHelper3dCube.geometry.vertices[2].z = wallHelperRectangle.segments[1].point.y),
              (wallHelper3dCube.geometry.vertices[5].x = wallHelperRectangle.segments[2].point.x),
              (wallHelper3dCube.geometry.vertices[5].z = wallHelperRectangle.segments[2].point.y),
              (wallHelper3dCube.geometry.vertices[7].x = wallHelperRectangle.segments[2].point.x),
              (wallHelper3dCube.geometry.vertices[7].z = wallHelperRectangle.segments[2].point.y),
              (wallHelper3dCube.geometry.vertices[4].x = wallHelperRectangle.segments[3].point.x),
              (wallHelper3dCube.geometry.vertices[4].z = wallHelperRectangle.segments[3].point.y),
              (wallHelper3dCube.geometry.vertices[6].x = wallHelperRectangle.segments[3].point.x),
              (wallHelper3dCube.geometry.vertices[6].z = wallHelperRectangle.segments[3].point.y),
              (wallHelper3dCube.geometry.verticesNeedUpdate = !0),
              (wallHelper3dCube.visible = !0),
              (tween = new TWEEN.Tween(controls.target).to(wallHelper3dCube.position, 500).onUpdate(render).start()),
              snapPointOverride.id && ((snapPointOverride = {}), setEndDrawingWalls())
          }
        ...
        if (2 === e.event.buttons) mouseMode = -1
          ...
          if (((mouseMode = 0), Date.now() - lastNewRoofSegmentClick > 250)) {
            if (
              ...
              recalcAllRoofSegmentsOnOtherLevels(roofIdCounter, project.activeLayer.data.id),
              startedDrawingRoofs)
            ) {
              ...
              try {
                ...
                ;(f.data.type = 'roofRectangle'),
                  ...
                  roofsGroup[project.activeLayer.data.id].addChild(roofsRectangles[roofPath.data.id])
                ...
                ;(h.position.x = roofHelper3dCube.position.x),
                  ...
                  roofPath.segments.length > 2 && alert('problem to many segments'),
                  (roofPath = new paper.Path()),
                  (roofPath.strokeColor = new paper.Color(0, 0, 0, 0)),
            ...
            ;(roofHelperPath.segments[0].point = snapPoint),
              ...
              (roofHelper3dCube.geometry.vertices[1].x = roofHelperRectangle.segments[0].point.x),
              (roofHelper3dCube.geometry.vertices[1].z = roofHelperRectangle.segments[0].point.y),
              (roofHelper3dCube.geometry.vertices[3].x = roofHelperRectangle.segments[0].point.x),
              (roofHelper3dCube.geometry.vertices[3].z = roofHelperRectangle.segments[0].point.y),
              (roofHelper3dCube.geometry.vertices[0].x = roofHelperRectangle.segments[1].point.x),
              (roofHelper3dCube.geometry.vertices[0].z = roofHelperRectangle.segments[1].point.y),
              (roofHelper3dCube.geometry.vertices[2].x = roofHelperRectangle.segments[1].point.x),
              (roofHelper3dCube.geometry.vertices[2].z = roofHelperRectangle.segments[1].point.y),
              (roofHelper3dCube.geometry.vertices[5].x = roofHelperRectangle.segments[2].point.x),
              (roofHelper3dCube.geometry.vertices[5].z = roofHelperRectangle.segments[2].point.y),
              (roofHelper3dCube.geometry.vertices[7].x = roofHelperRectangle.segments[2].point.x),
              (roofHelper3dCube.geometry.vertices[7].z = roofHelperRectangle.segments[2].point.y),
              (roofHelper3dCube.geometry.vertices[4].x = roofHelperRectangle.segments[3].point.x),
              (roofHelper3dCube.geometry.vertices[4].z = roofHelperRectangle.segments[3].point.y),
              (roofHelper3dCube.geometry.vertices[6].x = roofHelperRectangle.segments[3].point.x),
              (roofHelper3dCube.geometry.vertices[6].z = roofHelperRectangle.segments[3].point.y)
            ...
            ;(roofHelper3dCube.geometry.vertices[0].y = v - w),
              ...
              (tween = new TWEEN.Tween(controls.target).to(roofHelper3dCube.position, 500).onUpdate(render).start()),
              snapPointOverride.id && ((snapPointOverride = {}), setEndDrawingRoofs())
          }
        ...
        t && t.item.data
          ? 'background' === t.item.data.type
            ...
            ? (mouseMode = 2)
            : 'verticalGuide' === t.item.data.type
            ? (console.log(t.item.data.type), (selectedGuideId = t.item.data.id), (mouseMode = 9))
            : 'horizontalGuide' === t.item.data.type &&
              (console.log(t.item.data.type), (selectedGuideId = t.item.data.id), (mouseMode = 10))
          : (mouseMode = -1)
            ...
            Date.now() - lastNewFloorSegmentClick > 250 &&
              (startedDrawingFloor
                ...
                : ((startedDrawingFloor = !0),
                  ...
                  floorsGroup[project.activeLayer.data.id].addChild(Floors[floorIdCounter]),
                  (floorHelperPath.segments[0].point = snapPoint),
                  (floorHelperPath.segments[1].point = snapPoint),
                  (floorHelperPath.visible = !0))),
        ...
        if (2 === e.event.buttons) mouseMode = -1
          ...
          if (1 === dimensionPath.segments.length) dimensionPath.add(snapPoint)
            ...
            let s = dimensionPath.segments[1].point.subtract(dimensionPath.segments[0].point),
              R = new paper.Path()
        ...
        if (2 === e.event.buttons) mouseMode = -1
          ...
          ;(k.fontFamily = 'Courier New'),
            ...
            updateObjectPropertiesWindow(),
            (document.getElementById('textValueProp').style.backgroundColor = '#4e4e4e'),
    ...
    (tools.onMouseUp = function (e) {
      0 === mouseMode && dragging
        ...
        : 3 === mouseMode
        ? (snapPointOverride.id &&
            ((selectedItem.segments[movePointIconSelectedId].point.x = snapPointOverride.x),
            (selectedItem.segments[movePointIconSelectedId].point.y = snapPointOverride.y),
            (snapPointOverride = {})),
          relinkWallReferences(project.activeLayer.data.id),
          updatePlanHistory(
            plan,
        ...
        ? ((scalingY = !1),
          ...
          (clickableObjects[selectedItem.data.id].userData.height += stretchYPath.length),
          updatePlanHistory(
            plan,
            null,
            selectedItem.data.id,
    ...
    (tools.onMouseDrag = function (e) {
      ...
      if (mouseMode === -1) (paper.view.center = paper.view.center.add(t)), redrawGrid()
      else if (9 === mouseMode)
        verticalGuides[selectedGuideId].position.x = parseInt(e.point.x / snapTolerance) * snapTolerance
      else if (10 === mouseMode)
        horizontalGuides[selectedGuideId].position.y = parseInt(e.point.y / snapTolerance) * snapTolerance
        ...
        if (0 === mouseMode) {
          if (
            ...
            if (selectedItem.useMask) {
              ...
              if (
                (Object.keys(Walls).forEach(function (e) {
                  let t = Walls[e]
                  if ('object' === typeof t && t.data.level === project.activeLayer.data.id) {
                ...
                new Path.Circle({
                  ...
                  selectedItem.data.toolsRectangleInner && selectedItem.data.toolsRectangleInner.remove(),
                ...
                ;(selectedItem.rotation = selectedItem.data.angle),
                  ...
                  applyMasksToWall(l, wallsRectangles3d[l], project.activeLayer.data.id)
            ...
            if (null === o) {
              ...
              Object.keys(verticalGuides).forEach(function (e) {
                snapPoint.x >= verticalGuides[e].position.x - 10 &&
                  snapPoint.x <= verticalGuides[e].position.x + 10 &&
                  (r = new paper.Point(verticalGuides[e].position.x, snapPoint.y))
              }),
                Object.keys(horizontalGuides).forEach(function (e) {
                  snapPoint.y >= horizontalGuides[e].position.y - 10 &&
                    ...
                    (r
                      ...
                      : (r = new paper.Point(snapPoint.x, horizontalGuides[e].position.y)))
                }),
                r &&
                  ((snapPoint = r),
                  new Path.Circle({
            ...
            ;(selectedItem.position = snapPoint),
              (toolsGroup.position = snapPoint),
              (selectedItem.data.toolsRectangleInner.position = selectedItem.position)
            let p = snapPoint.x,
              m = snapPoint.y
            selectedItem.data.id &&
          ...
          if (selectedItem.data.id) {
            ...
            try {
              let v = (selectedItem.data.angle / 90) * Math.PI,
                w = e.point,
                I = selectedItem.position.subtract(e.point.subtract(selectedItem.position)),
                P = w.subtract(I),
                b = P.angleInRadians - v,
                x = P.length * Math.cos(b),
                R = P.length * Math.sin(b),
                M = new paper.Point(selectedItem.position.x + x / 2, selectedItem.position.y - R / 2),
                k = new paper.Point(selectedItem.position.x - x / 2, selectedItem.position.y + R / 2)
              ...
              if (Math.abs(T) >= 1e-5 && Math.abs(H) >= 1e-5) {
                ;(clickableObjects[selectedItem.data.id].scale.x =
                  T / (clickableObjects[selectedItem.data.id].userData.width * selectedItem.data.flipX)),
                  (clickableObjects[selectedItem.data.id].scale.z =
                    H / (clickableObjects[selectedItem.data.id].userData.depth * selectedItem.data.flipZ)),
                  maskObjects[selectedItem.data.id] &&
          ...
          e: for (let L = 0; L < unjoinedWallSegments.length; L++)
            if (
              ...
              ;(snapPoint = new paper.Point(unjoinedWallSegments[L].x, unjoinedWallSegments[L].y)),
                new Path.Circle({
                  center: snapPoint,
                  radius: screenScale / 2,
                  fillColor: new paper.Color(1, 0.3, 0.5, 0.75),
          ...
          if (!snapPointOverride.id)
            e: for (let L = 0; L < allWallSegments.length; L++)
              if (
                ...
                ;(snapPoint = new paper.Point(allWallSegments[L].x, allWallSegments[L].y)),
                  new Path.Circle({
                    center: snapPoint,
                    radius: screenScale / 2,
                    fillColor: new paper.Color(1, 0.3, 0.5, 0.75),
            ...
            (Object.keys(verticalGuides).forEach(function (o) {
              e.point.x >= verticalGuides[o].position.x - z &&
                ...
                null === r &&
                  ...
                  e.point.x <= verticalGuides[o].position.x - t + z &&
                  (r = new paper.Point(verticalGuides[o].position.x - t, e.point.y)),
                null === r &&
                  e.point.x >= verticalGuides[o].position.x + t - z &&
                  e.point.x <= verticalGuides[o].position.x + t + z &&
                  (r = new paper.Point(verticalGuides[o].position.x + t, e.point.y))
          ...
          if (selectedItem.data.id)
            try {
              ...
              let u = stretchYPath.segments[1].point.subtract(stretchYPath.segments[0].point),
                O = stretchYPath.length
              u.angle > 0 && (O *= -1),
                (clickableObjects[selectedItem.data.id].scale.y =
                  (stretchYStartHeight + O) / clickableObjects[selectedItem.data.id].userData.height),
                maskObjects[selectedItem.data.id] &&
                  (maskObjects[selectedItem.data.id].scale.y = Math.abs(
                    clickableObjects[selectedItem.data.id].scale.y
                  )),
                drawHeight(stretchYPath.segments[0].point, stretchYPath.segments[1].point, stretchYStartHeight),
                render(),
          ...
          if (selectedItem.data.id)
            try {
              ...
              let u = elevatePath.segments[1].point.subtract(elevatePath.segments[0].point),
                O = elevatePath.length
              u.angle > 0 && (O *= -1),
                (clickableObjects[selectedItem.data.id].position.y = elevateStartHeight + O),
                maskObjects[selectedItem.data.id] &&
                  (maskObjects[selectedItem.data.id].position.y = elevateStartHeight + O),
                drawHeight(elevatePath.segments[0].point, elevatePath.segments[1].point, elevateStartHeight),
                (controls.target.y = clickableObjects[selectedItem.data.id].position.y),
                render(),
            ...
            Object.keys(horizontalGuides).forEach(function (t) {
              e.point.y >= horizontalGuides[t].position.y - 10 &&
                ...
                (r
                  ...
                  : (r = new paper.Point(e.point.x, horizontalGuides[t].position.y)))
            }),
            r &&
              ((snapPoint = r),
              new Path.Circle({
          ...
          7 === mouseMode &&
            ...
            (document.getElementById('textXProp').value = snapPoint.x.toFixed(3)),
            (document.getElementById('textYProp').value = snapPoint.y.toFixed(3)))
      else if ('background' === toolMode)
        if (2 === e.event.buttons) (paper.view.center = paper.view.center.add(t)), redrawGrid()
        else if (0 === mouseMode) {
          ;(dragging = !0),
            (snapPoint = e.point),
            (snapPoint.x = parseInt(e.point.x)),
          ...
          backgroundRaster &&
            ...
            (backgroundRaster.data.toolsRectangleInner.position = backgroundRaster.position),
            (resizeIcon.position = selectedItem.data.toolsRectangleInner.segments[3].point))
        } else if (2 === mouseMode) {
          scalingXY = !0
          try {
            if (e.point.x > 1 && e.point.y > 1) {
              ;(backgroundRasterRatioX = Math.abs(backgroundRaster.bounds.right / backgroundRaster.bounds.left)),
                (backgroundRasterRatioY = Math.abs(backgroundRaster.bounds.bottom / backgroundRaster.bounds.top))
              let S = new paper.Point(-e.point.x / backgroundRasterRatioX, -e.point.y / backgroundRasterRatioY),
                D = e.point
              ;(backgroundRaster.data.toolsRectangleInner.bounds = new Rectangle(S, D)),
                (backgroundRaster.bounds.width = backgroundRaster.data.toolsRectangleInner.bounds.width),
                (backgroundRaster.bounds.height = backgroundRaster.data.toolsRectangleInner.bounds.height),
                (backgroundRaster.position.x = backgroundRaster.data.toolsRectangleInner.position.x),
                (backgroundRaster.position.y = backgroundRaster.data.toolsRectangleInner.position.y),
                (resizeIcon.position = backgroundRaster.data.toolsRectangleInner.segments[3].point)
            }
          } catch (e) {
            console.log(e)
      ...
      if (((lastMousePoint = e.point), 'walls' === toolMode)) {
        if (((snapPoint = null), startedDrawingWalls)) {
          ...
          ;(wallHelperRectangle.segments[0].point = new Point(
            ...
            (wallHelperPath.segments[1].point = snapPoint),
            drawLength(wallHelperPath.segments[0].point, wallHelperPath.segments[1].point, n < 0 ? -1 : 1),
            (wallHelper3dCube.geometry.vertices[1].x = wallHelperRectangle.segments[0].point.x),
            (wallHelper3dCube.geometry.vertices[1].z = wallHelperRectangle.segments[0].point.y),
            (wallHelper3dCube.geometry.vertices[3].x = wallHelperRectangle.segments[0].point.x),
            (wallHelper3dCube.geometry.vertices[3].z = wallHelperRectangle.segments[0].point.y),
            ...
            (wallHelper3dCube.geometry.vertices[6].z = wallHelperRectangle.segments[3].point.y),
            (wallHelper3dCube.geometry.verticesNeedUpdate = !0),
            (tween = new TWEEN.Tween(controls.target).to(wallHelper3dCube.position, 1).onUpdate(render).start())
        }
        snapPointOverride = {}
        let l = null,
          i = null,
        ...
        if (
          (snapPointOverride.id ||
            (Object.keys(verticalGuides).forEach(function (t) {
              e.point.x >= verticalGuides[t].position.x - s &&
                e.point.x <= verticalGuides[t].position.x + s &&
                ((l = new paper.Point(verticalGuides[t].position.x, e.point.y)), (i = verticalGuides[t].data.id)),
                null === l &&
                  e.point.x >= verticalGuides[t].position.x - r - s &&
                  e.point.x <= verticalGuides[t].position.x - r + s &&
                  ((l = new paper.Point(verticalGuides[t].position.x - r, e.point.y)), (i = verticalGuides[t].data.id)),
                null === l &&
                  e.point.x >= verticalGuides[t].position.x + r - s &&
                  e.point.x <= verticalGuides[t].position.x + r + s &&
                  ((l = new paper.Point(verticalGuides[t].position.x + r, e.point.y)), (i = verticalGuides[t].data.id))
            ...
            Object.keys(horizontalGuides).forEach(function (t) {
              ...
              e.point.y >= horizontalGuides[t].position.y - s &&
                ...
                null === o &&
                  ...
                  e.point.y <= horizontalGuides[t].position.y + r + s &&
                  (o = horizontalGuides[t].position.y + r),
                o && ((i = horizontalGuides[t].data.id), l ? (l.y = o) : (l = new paper.Point(e.point.x, o)))
            }),
            l &&
              ((snapPoint = l),
              new Path.Circle({
          ...
          e: for (let d = 0; d < unjoinedWallSegments.length; d++)
            if (
              ...
              ;(snapPoint = new paper.Point(unjoinedWallSegments[d].x, unjoinedWallSegments[d].y)),
                new Path.Circle({
                  center: snapPoint,
                  radius: screenScale / 2,
                  fillColor: new paper.Color(1, 0.3, 0.5, 0.75),
        ...
        if (!snapPointOverride.id)
          e: for (let d = 0; d < allWallSegments.length; d++)
            if (
              ...
              ;(snapPoint = new paper.Point(allWallSegments[d].x, allWallSegments[d].y)),
                new Path.Circle({
                  center: snapPoint,
                  radius: screenScale / 2,
                  fillColor: new paper.Color(1, 0.3, 0.5, 0.75),
        ...
        if (startedDrawingRoofs) {
          ...
          ;(roofHelperRectangle.segments[0].point = new Point(
            ...
            (roofHelperPath.segments[1].point = snapPoint),
            drawLength(roofHelperPath.segments[0].point, roofHelperPath.segments[1].point, n < 0 ? -1 : 1),
            (roofHelper3dCube.geometry.vertices[1].x = roofHelperRectangle.segments[0].point.x),
            (roofHelper3dCube.geometry.vertices[1].z = roofHelperRectangle.segments[0].point.y),
            (roofHelper3dCube.geometry.vertices[3].x = roofHelperRectangle.segments[0].point.x),
            (roofHelper3dCube.geometry.vertices[3].z = roofHelperRectangle.segments[0].point.y),
          ...
          ;(roofHelper3dCube.geometry.vertices[0].y = c - u),
            ...
            (roofHelper3dCube.geometry.verticesNeedUpdate = !0),
            (tween = new TWEEN.Tween(controls.target).to(roofHelper3dCube.position, 1).onUpdate(render).start())
        }
        snapPointOverride = {}
        let l = null,
          i = null,
          ...
          Object.keys(horizontalGuides).forEach(function (t) {
            e.point.y >= horizontalGuides[t].position.y - 10 &&
              ...
              (l
                ...
                : (l = new paper.Point(e.point.x, horizontalGuides[t].position.y)),
        ...
        if (startedDrawingDimension)
          if (1 === dimensionPath.segments.length)
            (dimensionHelperPath.segments[1].point = snapPoint),
              drawDimension(dimensionHelperPath.segments[0].point, dimensionHelperPath.segments[1].point, 10)
          else {
            let t = dimensionHelperPath.segments[1].point.subtract(dimensionHelperPath.segments[0].point),
              p = dimensionHelperPath.segments[1].point.subtract(snapPoint),
              m = (p.angle - t.angle + 360) % 360,
              g = (m / 180) * Math.PI,
              y = p.length * Math.sin(g)
            drawDimension(dimensionHelperPath.segments[0].point, dimensionHelperPath.segments[1].point, y)
          }
      }
    }),
    planCanvas.addEventListener(
      ...
      function (e) {
        'pointer' === toolMode
          ...
          : 'floor' === toolMode &&
            startedDrawingFloor &&
            ((startedDrawingFloor = !1), (floorHelperPath.visible = !1), (floorPath.closed = !0))
      },
      !1
    ),
...
function setEndDrawingDimension() {
  startedDrawingDimension && ((startedDrawingDimension = !1), (dimensionHelperPath.visible = !1))
}
function setEndDrawingFloors() {
  startedDrawingFloor &&
    ((startedDrawingFloor = !1),
    (floorHelperPath.visible = !1),
    Floors[floorPath.data.id].segments.length < 3 ? deleteFloorByKey(floorPath.data.id) : (floorPath.closed = !0))
}
...
function drawAngle(e, t, o, a) {
  ...
  let l = avgAngleRad(e, t),
    i = ((t - e + 2 * Math.PI) % (2 * Math.PI), o.add(new Point(-Math.cos(-e) * n, Math.sin(-e) * n))),
    r = (180 * (e - t)) / Math.PI
  ;(r = 180 - r), (r = (r + 360) % 360)
  let s
  r < 180
    ? (s = o.add(new Point(Math.sin(l) * n, -Math.cos(l) * n)))
    : ((s = o.add(new Point(-Math.sin(l) * n, Math.cos(l) * n))), (r = 360 - r)),
...
function drawHeight(e, t, o) {
  let a = 10,
    ...
    c = new paper.PointText({
      ...
      .removeOnMove()
      .removeOnDrag()
  ;(c.fontFamily = 'Courier New'), (c.fillColor = 'white'), (c.fontSize = screenScale / 1.5), c.rotate(s)
  let u = o + n.length
...
function compareAnglesRadians(e, t) {
  let o = t - e
  return o < -Math.PI && (o += 2 * Math.PI), o > Math.PI && (o -= 2 * Math.PI), o > 0 ? 1 : o < 0 ? -1 : 0
}
...
function avgAngleRad(e, t) {
  return Math.atan2((Math.sin(e) + Math.sin(t)) / 2, (Math.cos(e) + Math.cos(t)) / 2)
}
  ...
  if (2 === e.segments.length) {
    ...
    if (e.data.join0.id) {
      ...
      if (0 === e.data.join0.seg) {
        ...
        s /= 2
        let d = r + Math.PI / 2,
          c = Math.sin(r) * t + Math.sin(d) * (Math.tan(s) * Math.atan(Math.PI / 2) * t),
          u = -(Math.cos(r) * t) - Math.cos(d) * (Math.tan(s) * Math.atan(Math.PI / 2) * t),
          p = Math.sin(r) * a + Math.sin(d) * (Math.tan(s) * Math.atan(Math.PI / 2) * a),
          m = -(Math.cos(r) * a) - Math.cos(d) * (Math.tan(s) * Math.atan(Math.PI / 2) * a),
    ...
    ;(x[0].y = -b + e.data.height[1]),
      ...
      (wallsRectangles3d[e.data.id].position.y = b + project.layers[e.data.level].data.height)
...
function reDrawRoofCorners(e) {
  if (2 === e.segments.length) {
    ...
    ;(R[0].y = b - x),
      ...
      (roofsRectangles3d[e.data.id].position.y = b + x + project.layers[e.data.level].data.height + e.data.startHeight)
  } else console.log('segments lenght was not 2')
}
  ...
  if (selectedItem && selectedItem.data && selectedItem.data.type)
    switch (selectedItem.data.type) {
      case 'furniture':
        ...
        try {
          ;(document.getElementById('objectId').innerText = selectedItem.data.id),
            (document.getElementById('objectName').innerText = camelCaseToSentence(selectedItem.data.fid)),
            (document.getElementById('furnitureAngleProp').innerText = selectedItem.data.angle.toFixed(2)),
            (document.getElementById('furnitureXProp').style.backgroundColor = '#4e4e4e'),
            (document.getElementById('furnitureXProp').value =
              clickableObjects[selectedItem.data.id].position.x.toFixed(3)),
            (document.getElementById('furnitureYProp').style.backgroundColor = '#4e4e4e'),
            (document.getElementById('furnitureYProp').value =
              clickableObjects[selectedItem.data.id].position.y.toFixed(3)),
            (document.getElementById('furnitureZProp').style.backgroundColor = '#4e4e4e'),
            (document.getElementById('furnitureZProp').value =
              clickableObjects[selectedItem.data.id].position.z.toFixed(3)),
            (document.getElementById('furnitureWidthProp').style.backgroundColor = '#4e4e4e'),
            (document.getElementById('furnitureWidthProp').value = (
              clickableObjects[selectedItem.data.id].userData.width * clickableObjects[selectedItem.data.id].scale.x
            ).toFixed(3)),
            (document.getElementById('furnitureDepthProp').style.backgroundColor = '#4e4e4e'),
            (document.getElementById('furnitureDepthProp').value = (
              clickableObjects[selectedItem.data.id].userData.depth * clickableObjects[selectedItem.data.id].scale.z
            ).toFixed(3)),
            (document.getElementById('furnitureHeightProp').style.backgroundColor = '#4e4e4e'),
            (document.getElementById('furnitureHeightProp').value = (
              clickableObjects[selectedItem.data.id].userData.height * clickableObjects[selectedItem.data.id].scale.y
            ).toFixed(3)),
            (document.getElementById('flipX').checked = 1 !== selectedItem.data.flipX),
            (document.getElementById('flipZ').checked = 1 !== selectedItem.data.flipZ),
            (document.getElementById('furnitureLevelProp').innerText = selectedItem.data.level)
        } catch (e) {
      ...
      case 'wallPath':
        setPropertiesView('wallPath'),
          (document.getElementById('wallIdHidden').value = selectedItem.data.id),
          (document.getElementById('wallIdProp').innerText = selectedItem.data.id),
          (document.getElementById('wallHeightProp').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('wallHeightProp').value = selectedItem.data.height[0]),
          (document.getElementById('wallHeight0Prop').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('wallHeight0Prop').value = selectedItem.data.height[0]),
          (document.getElementById('wallHeight1Prop').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('wallHeight1Prop').value = selectedItem.data.height[1]),
          (document.getElementById('wallThicknessProp').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('wallThicknessProp').value = selectedItem.data.thickness),
          (document.getElementById('wallLevelProp').innerText = selectedItem.data.level)
        break
      case 'roofPath':
        setPropertiesView('roofPath'),
          (document.getElementById('roofIdHidden').value = selectedItem.data.id),
          (document.getElementById('roofIdProp').innerText = selectedItem.data.id),
          (document.getElementById('roofThicknessProp').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('roofThicknessProp').value = selectedItem.data.thickness),
          (document.getElementById('roofRiseProp').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('roofRiseProp').value = selectedItem.data.rise),
          (document.getElementById('roofStartHeightProp').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('roofStartHeightProp').value = selectedItem.data.startHeight),
          (document.getElementById('roofWidthProp').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('roofWidthProp').value = selectedItem.data.width),
          (document.getElementById('roofLevelProp').innerText = selectedItem.data.level),
          updateExtraRoofInfo()
        break
      case 'floor':
        setPropertiesView('floor'),
          (document.getElementById('floorIdProp').innerText = selectedItem.data.id),
          (document.getElementById('floorAreaProp').innerHTML =
            Math.abs(selectedItem.area / 1e4).toFixed(3) + ' M&sup2;'),
          (document.getElementById('floorThicknessProp').innerText = selectedItem.data.thickness),
          (document.getElementById('floorLevelProp').innerText = selectedItem.data.level)
        break
      case 'dimension':
        setPropertiesView('dimension'),
          (document.getElementById('dimensionIdProp').innerText = selectedItem.data.id),
      ...
      case 'text':
        setPropertiesView('text'),
          (document.getElementById('textIdProp').innerText = selectedItem.data.id),
          (document.getElementById('textValueProp').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('textValueProp').value = selectedItem.data.value),
          (document.getElementById('textXProp').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('textXProp').value = selectedItem.data.x.toFixed(3)),
          (document.getElementById('textYProp').style.backgroundColor = '#4e4e4e'),
          (document.getElementById('textYProp').value = selectedItem.data.y.toFixed(3)),
          (document.getElementById('textLevelProp').innerText = selectedItem.data.level)
        break
      case 'planView':
      ...
      case 'defaults':
        ...
        break
      default:
        alert('set properties view for ' + selectedItem.data.type + ' not implemented')
    }
...
function setPropertiesView(e) {
  switch (
    ...
    (document.getElementById('furniture3DModelPropertiesView').style.display = 'none'),
    ...
    case 'model3dMeta':
      document.getElementById('furniture3DModelPropertiesView').style.display = 'block'
      break
    case 'furniture':
...
function updateDefaultWallHeight(e) {
  ...
  let t = new THREE.BoxGeometry(defaultWallThickness, defaultWallHeight, defaultWallThickness),
  ...
  ;(wallHelper3dCube = new THREE.Mesh(t, o)),
    (wallHelper3dCube.position.y = defaultWallHeight / 2 + project.activeLayer.data.height),
    (wallHelper3dCube.visible = !1),
    scene.add(wallHelper3dCube)
}
...
function updateDefaultRoofThickness(e) {
  ...
  let t = new THREE.BoxGeometry(defaultRoofWidth, defaultRoofThickness, defaultRoofWidth),
    o = new THREE.MeshStandardMaterial({
      color: 0,
      transparent: !0,
      opacity: 0.25,
  ...
  ;(roofHelper3dCube = new THREE.Mesh(t, o)),
    (roofHelper3dCube.position.y = defaultRoofThickness / 2 + project.activeLayer.data.height),
    (roofHelper3dCube.visible = !1),
    scene.add(roofHelper3dCube),
    updateExtraDefaultRoofInfo()
}
...
function updateDefaultRoofStartHeight(e) {
  ...
  ;(roofHelper3dCube = new THREE.Mesh(t, o)),
    (roofHelper3dCube.position.y = defaultRoofThickness / 2 + project.activeLayer.data.height + defaultRoofStartHeight),
    (roofHelper3dCube.visible = !1),
    scene.add(roofHelper3dCube),
    updateExtraDefaultRoofInfo()
}
...
function updateExtraDefaultRoofInfo() {
  ...
  ;(document.getElementById('defaultRafterLengthProp').innerText = o.toFixed(2)),
...
function newLevel() {
  ...
  if (
    ...
    plan.levels[e] = {
      id: e.toString(),
      height: parseInt(plan.levels[e] ? plan.levels[e].height : e * defaultWallHeight),
    }
  }
    ...
    levelButtons
      ...
      : ((levelButtons = {}),
        'planView' === UILayout
          ...
          : ((t.style.left = '354px'), (t.style.bottom = '50%'), (t.style.marginBottom = '26px'))),
    'planView' !== UILayout && (t.style.marginBottom = '26px'),
    (t.style.marginLeft = 104 * e + 'px'),
    '3dView' !== UILayout &&
      ((t.style.display = 'block'),
  ...
  project.layers.forEach(function (o) {
    o.data &&
      ...
      (wallsGroup[o.data.id] &&
        (otherLayerWallsRasters.push(wallsGroup[o.data.id].rasterize(view.resolution, !1)),
        (otherLayerWallsRasters[otherLayerWallsRasters.length - 1].data.level = o.data.id)),
      furnitureGroup[o.data.id] &&
        (otherLayerFurnitureRasters.push(furnitureGroup[o.data.id].rasterize(view.resolution, !1)),
        (otherLayerFurnitureRasters[otherLayerFurnitureRasters.length - 1].data.level = o.data.id))),
      (o.visible = !1),
    ...
    gridGroup.sendToBack(),
    backgroundRaster &&
      (project.activeLayer.addChild(backgroundRaster), backgroundRaster.sendToBack(), gridGroup.sendToBack()),
    (project.activeLayer.visible = !0),
    (wallHelper3dCube.position.y = defaultWallHeight / 2 + project.activeLayer.data.height),
    (roofHelper3dCube.position.y = defaultRoofThickness / 2 + project.activeLayer.data.height),
    setPropertiesView('level'),
    (document.getElementById('levelIdProp').innerHTML = project.activeLayer.data.id),
    (document.getElementById('levelNameProp').innerHTML = project.activeLayer.name),
    (document.getElementById('levelHeightProp').style.backgroundColor = '#4e4e4e'),
    (document.getElementById('levelHeightProp').value = project.activeLayer.data.height)
}
...
function redrawFloor(e) {
  ...
  a.position.set(0, project.layers['level' + e.data.level].data.height + e.data.thickness, 0),
...
function showFurnitureLicenseSummary(e) {
  ...
  ;(document.getElementById('model3dLicense').innerHTML = a),
    (document.getElementById('model3dLargeThumb').src = 'objects/' + e + '.png'),
...
function hideMouseIndicators() {
  mouseIndicatorX &&
    ((modalsActive = !0), (mouseIndicatorX.style.display = 'none'), (mouseIndicatorY.style.display = 'none'))
}
function showMouseIndicators() {
  mouseIndicatorX &&
    '3dView' !== UILayout &&
    ((modalsActive = !1), (mouseIndicatorX.style.display = 'block'), (mouseIndicatorY.style.display = 'block'))
}
...
function updateFurniturePosX(e) {
  if (selectedItem.data.id) {
    ...
    ;(selectedItem.position.x = t),
      ...
      maskObjects[selectedItem.data.id] && (maskObjects[selectedItem.data.id].position.x = t),
      (tween = new TWEEN.Tween(controls.target)
...
function updateFurniturePosZ(e) {
  if (selectedItem.data.id) {
    ...
    ;(selectedItem.position.y = t),
      ...
      maskObjects[selectedItem.data.id] && (maskObjects[selectedItem.data.id].position.z = t),
      (tween = new TWEEN.Tween(controls.target)
        .to(clickableObjects[selectedItem.data.id].position, 100)
        .onUpdate(render)
        .start()),
...
function updateFurniturePosY(e) {
  if (selectedItem.data.id) {
    ...
    ;(clickableObjects[selectedItem.data.id].position.y = t),
      maskObjects[selectedItem.data.id] && (maskObjects[selectedItem.data.id].position.y = t),
      (tween = new TWEEN.Tween(controls.target)
        .to(clickableObjects[selectedItem.data.id].position, 100)
        .onUpdate(render)
        .start()),
...
function updateFurnitureWidth(e) {
  if (selectedItem.data.id) {
    let t = parseFloat(e)
    ;(clickableObjects[selectedItem.data.id].scale.x = t / clickableObjects[selectedItem.data.id].userData.width),
      maskObjects[selectedItem.data.id] &&
        (maskObjects[selectedItem.data.id].scale.x = clickableObjects[selectedItem.data.id].scale.x)
...
function updateFurnitureDepth(e) {
  if (selectedItem.data.id) {
    let t = parseFloat(e)
    ;(clickableObjects[selectedItem.data.id].scale.z = t / clickableObjects[selectedItem.data.id].userData.depth),
      maskObjects[selectedItem.data.id] &&
        (maskObjects[selectedItem.data.id].scale.z = clickableObjects[selectedItem.data.id].scale.z)
    let o = selectedItem.position,
      a = selectedItem.rotation
    ;(selectedItem.rotation = 0),
...
function updateFurnitureHeight(e) {
  if (selectedItem.data.id) {
    let t = parseFloat(e)
    ;(clickableObjects[selectedItem.data.id].scale.y = t / clickableObjects[selectedItem.data.id].userData.height),
      maskObjects[selectedItem.data.id] &&
        (maskObjects[selectedItem.data.id].scale.y = clickableObjects[selectedItem.data.id].scale.y),
      render(),
      updatePlanHistory(
        plan,
        null,
...
function flipImageDataX(e) {
  for (
    let t = e.canvas.getContext('2d'), o = t.getImageData(0, 0, e.width, e.height), a = o.data, n = a.slice(), l = 0;
    l < e.height;
    l++
  )
    for (let i = 0; i < e.width; i++) {
      ...
      ;(a[r] = n[s]), (a[r + 1] = n[s + 1]), (a[r + 2] = n[s + 2]), (a[r + 3] = n[s + 3])
    }
...
function exportToObj() {
  try {
    ...
    Object.keys(wallsRectangles3d).forEach(function (e) {
      ...
      'object' === typeof o && (o.visible ? t.add(o.clone()) : t.add(maskObjectsApplied[e].clone()))
    }),
      Object.keys(roofsRectangles3d).forEach(function (e) {
        let o = roofsRectangles3d[e]
        'object' === typeof o && (o.visible ? t.add(o.clone()) : t.add(maskObjectsAppliedRoof[e].clone()))
      }),
      Object.keys(Floors3d).forEach(function (e) {
        let o = Floors3d[e]
    ...
    for (
      a.download = 'HomeIdea3D.obj',
        ...
        a.href = window.URL.createObjectURL(new Blob([o], { type: 'text/plain' })),
...
function setInterfacePropertiesFromPlan(e) {
  e.wallDiffuse &&
    ...
    'default' === UILayout && $('#wallDiffuse').minicolors('value', '#' + wallMaterial.color.getHexString())),
    e.wallOpacity &&
      ((wallMaterial.opacity = parseFloat(e.wallOpacity)),
      'default' === UILayout && $('#wallDiffuse').minicolors('opacity', parseFloat(e.wallOpacity))),
    e.wallSpecular &&
      ((wallMaterial.specular = new THREE.Color(e.wallSpecular)),
      'default' === UILayout && $('#wallSpecular').minicolors('value', '#' + wallMaterial.specular.getHexString())),
    e.roofDiffuse &&
      ((roofMaterial.color = new THREE.Color(e.roofDiffuse)),
      'default' === UILayout && $('#roofDiffuse').minicolors('value', '#' + roofMaterial.color.getHexString())),
    e.roofOpacity &&
      ((roofMaterial.opacity = parseFloat(e.roofOpacity)),
      'default' === UILayout && $('#roofDiffuse').minicolors('opacity', parseFloat(e.roofOpacity))),
    e.roofSpecular &&
      ((roofMaterial.specular = new THREE.Color(e.roofSpecular)),
      'default' === UILayout && $('#roofSpecular').minicolors('value', '#' + roofMaterial.specular.getHexString())),
    e.floorDiffuse &&
      ((floorMaterial.color = new THREE.Color(e.floorDiffuse)),
      'default' === UILayout && $('#floorDiffuse').minicolors('value', '#' + floorMaterial.color.getHexString())),
    e.floorOpacity &&
      ((floorMaterial.opacity = parseFloat(e.floorOpacity)),
      'default' === UILayout && $('#floorDiffuse').minicolors('opacity', parseFloat(e.floorOpacity))),
    e.floorSpecular &&
      ((floorMaterial.specular = new THREE.Color(e.floorSpecular)),
      'default' === UILayout && $('#floorSpecular').minicolors('value', '#' + floorMaterial.specular.getHexString())),
    e.groundDiffuse &&
      (e.groundDiffuse.startsWith('rgba')
        ? (groundMat.color = new THREE.Color(parseInt(rgba2hex(e.groundDiffuse))))
        : (groundMat.color = new THREE.Color(parseInt('0x' + e.groundDiffuse))),
      'default' === UILayout && $('#groundDiffuse').minicolors('value', '#' + groundMat.color.getHexString())),
    e.groundOpacity &&
      ((groundMat.opacity = parseFloat(e.groundOpacity)),
      'default' === UILayout && $('#groundDiffuse').minicolors('opacity', parseFloat(e.groundOpacity))),
    e.groundSpecular &&
      (e.groundSpecular.startsWith('rgba')
        ? (groundMat.specular = new THREE.Color(parseInt(rgba2hex(e.groundSpecular))))
        : (groundMat.specular = new THREE.Color(parseInt('0x' + e.groundSpecular))),
      'default' === UILayout && $('#groundSpecular').minicolors('value', '#' + groundMat.specular.getHexString())),
    e.depthWrite
      ? ((document.getElementById('depthWriteMode').checked = e.depthWrite), setDepthWriteMode(e.depthWrite))
      : ((document.getElementById('depthWriteMode').checked = !1), setDepthWriteMode(!1)),
    e.sortObjects
      ? ((document.getElementById('sortObjectsMode').checked = e.sortObjects), setSortObjectsMode(e.sortObjects))
      : ((document.getElementById('sortObjectsMode').checked = !1), setSortObjectsMode(!1)),
    e.azimuth &&
      ((azimuth = e.azimuth), (document.getElementById('sunAzimuth').value = azimuth), setSunAzimuth(e.azimuth)),
    e.inclination &&
...
function loadInViewThumbs() {
  $('img[realsrc]').each(function (e) {
    let t = $(this)
    t.position().top < $('#catalogItems').scrollTop() + $('#catalogItems').height() &&
...
function recenter3dview() {
  ...
  Object.keys(wallsRectangles3d).forEach(function (e) {
    ...
    (d = a + (n - a) / 2),
    (s = e / 2),
    new TWEEN.Tween(camera.position).to({ x: -954, y: 360, z: 517 }, 500).onUpdate(render).start(),
    new TWEEN.Tween(controls.target).to({ x: r, y: s, z: d }, 500).onUpdate(render).start()
}
function recenterPlanView() {
  ...
  let a = paper.view.bounds.width * paper.view.bounds.height,
    n = Math.max(zoomRectangle.bounds.width * zoomRectangle.bounds.height * 2, 1e5),
    l = Math.sqrt(a / n)
...
function updateGroundWidth(e) {
  ...
  let t = new THREE.PlaneGeometry(100 * groundWidth, 100 * groundLength)
  ;(groundMat = new THREE.MeshPhongMaterial({ transparent: !0, opacity: 1 })),
    (groundMat.color = new THREE.Color(2304293)),
    console.log('groundDiffuse = ' + document.getElementById('groundDiffuse').value),
    (groundMat.specular = new THREE.Color(15925148)),
